...
import useUserClient from "@/api/user/api";
...
export default function UserScreen({ userId }: UserScreenProps) {
  ...
  async function getHouseData(tenant_id: string) {
    let data = await userClient.fetchTenantData(tenant_id);
    if (data) {
      setHouseData([...data]);
    }
  }
  ...
  async function payRent(house_id: string, history: any[], rent: number) {
    async function pay(house_id: string, history: any[], rent: number) {
      await userClient.payRent(house_id, history, rent);
    }
  ...
  useEffect(() => {
    ...
    //   let data = await userClient.fetchHouseWithID(id);
        ...
        paymentOpen ? (
          paymentSuccess ? (
            ...
            <>
              ...
              />
              <button
                className="bg-gray-900 border border-black text-gray-300 text-sm w-fit px-8 mx-auto   py-3 rounded-full mt-4"
          ...
          <div className="flex flex-col gap-4">
            ...
            <p className="flex items-center gap-1 italic text-gray-300">
              <MapPin className="w-4 h-auto" />
              {houseData[0].house_address}
            </p>
            <h2 className="text-4xl font-semibold text-white ">Payment</h2>
            ...
            <div className="flex flex-col gap-4">
              <h2 className="text-4xl font-semibold  text-white ">History</h2>
            ...
            <div className="flex flex-col gap-4 mb-16">
              ...
              <div className="bg-gray-900 border border-gray-800 p-4 rounded-2xl shadow-2xl">
                {houseData[0].contacts ? (
                  houseData[0].contacts.map((contact, index: number) => (
                    <div
                      key={index}
                      className="flex justify-between items-center"
                    >
